...
% compute FP in the 1st layer
z2 = a1 * Theta1';
a2 = sigmoid(z2);

% transform the Z2 and sigmoid the matrix
a2 = [ones(m, 1) a2];
z3 = a2 * Theta2';
a3 = sigmoid(z3);

% compute cost function with vectorization
for (i = 1: m)
	y_true = zeros(1, size(a3)(2));
	y_true(y(i)) = 1;
	J += - y_true * log(a3(i, :)') - (1 - y_true) * log(1 - a3(i, :)');
end
J = J / m;
